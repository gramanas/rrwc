...
ProfileParser::ProfileParser(const QVector<Output *> &outputs,
  ...
  for (const auto& output : outputs) {
    ...
    p_output->comment = output->comment;
    p_output->commentText = output->commentText;
    p_output->index = output->index;
    m_outputs << p_output;
  }
}

ProfileParser::ProfileParser(Logger *logger)
  : p_logger(logger) {
}
...
void ProfileParser::writeToFile(const QString &filename) {
  ...
  if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
    p_logger->err(ERR_PROFILE_SAVE.arg(filename));
    return;
  }
  ...
  for (const auto *output : m_outputs) {
    ...
    out << OTP_COMMENT << "=" << (output->comment ? "true" : "false") << "\n";
    out << OTP_COMMENT_TEXT << "= \"" << output->commentText << "\"" << "\n";
    out << "}\n";
  }
  file.close();
}
...
bool ProfileParser::readFromFile(const QString &filename, QVector<Output *> &vector) {
  ...
  while (!in.atEnd()) {
    ...
    if (line.startsWith('#')) {
      ...
      if (p_output != nullptr) {
        p_logger->err(ERR_PROFILE.arg(filename) + ERR_PROFILE_MISSING_BRKT);
        return false;
      }
      p_output = new Output;
      ...
      } else if (data[0] == OTP_COMMENT) {
        parse(p_output->comment, data);
      } else if (data[0] == OTP_COMMENT_TEXT) {
        parse(p_output->commentText, data);
      } else {
        p_logger->err(ERR_PROFILE.arg(filename) + line);
        return false;
      }
    }
  }
